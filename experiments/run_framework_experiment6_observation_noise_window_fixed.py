...
class ExperimentConfig:
    ...
    single_initial_states_per_system: int = 10
    # Match Experiment 3 exactly for the corrected multi-filter baseline.
    multi_initial_states_per_system: int = 20
    ...
    # Multi-filter case: EXACT Experiment-3 moderate-gap/balanced underlying
    # system.  Five leading modes remain in the trajectory, even though only
    # q1,q2,q3 are evaluated in this noise experiment.
    multi_system_leading_eigenvalues: Tuple[float, ...] = (0.96, 0.92, 0.88, 0.84, 0.80)
    multi_system_target_modes: int = 5
    multi_tail_min: float = 0.20
    multi_tail_max: float = 0.90
    multi_tail_gap_below_last_target: float = 0.01
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--initial-states-per-system", type=int, default=10,
        help="Single-filter initial states per system; used only if single filters are rerun.",
    )
    p.add_argument(
        "--multi-initial-states-per-system", type=int, default=20,
        help="Corrected multi-filter initial states per system. Default 20 matches Experiment 3.",
    )
    p.add_argument("--noise-realizations", type=int, default=2)
    p.add_argument(
        "--reuse-single-results-dir",
        type=Path,
        default=None,
        help=(
            "Existing Experiment-6 results directory. When supplied, its single-filter "
            "trajectory_stage_outcomes.csv and base_trajectories.csv are reused and ONLY "
            "the corrected multi-filter branch is simulated."
        ),
    )
...
def config_from_args(args: argparse.Namespace) -> ExperimentConfig:
    return ExperimentConfig(
        ...
        single_initial_states_per_system=args.initial_states_per_system,
        multi_initial_states_per_system=args.multi_initial_states_per_system,
...
def validate_config(cfg: ExperimentConfig) -> None:
    ...
    if (
        cfg.system_replicates < 1
        or cfg.single_initial_states_per_system < 1
        or cfg.multi_initial_states_per_system < 1
    ):
    ...
    mags = np.abs(np.asarray(cfg.multi_system_leading_eigenvalues, dtype=float))
    if len(mags) != cfg.multi_system_target_modes:
        raise ValueError("Corrected multi-filter system must contain five designated leading modes.")
    if not np.all(mags[:-1] > mags[1:]):
        raise ValueError("multi_system_leading_eigenvalues must have strictly decreasing magnitudes.")
    if cfg.multi_n_directions > cfg.multi_system_target_modes:
        raise ValueError("Cannot evaluate more filters than designated system modes.")
    available_tail_max = min(
        cfg.multi_tail_max,
        float(mags[-1]) - cfg.multi_tail_gap_below_last_target,
    )
    if available_tail_max <= cfg.multi_tail_min:
        raise ValueError("No valid tail interval remains below the fifth designated mode.")
...
def build_multi_system(
    ...
    """Reproduce Experiment 3's moderate-gap normal-system construction."""
    rng = np.random.default_rng(system_seed)
    G = rng.normal(size=(cfg.dim, cfg.dim))
    Q, _ = np.linalg.qr(G)

    leading = np.asarray(cfg.multi_system_leading_eigenvalues, dtype=float)
    available_tail_max = min(
        cfg.multi_tail_max,
        abs(float(leading[-1])) - cfg.multi_tail_gap_below_last_target,
    )
    remaining = cfg.dim - cfg.multi_system_target_modes

    # Exact Experiment-3 mapping: draw U(0,1), map into the admissible tail
    # interval, sort magnitudes, then assign random signs.
    u = rng.uniform(0.0, 1.0, size=remaining)
    tail_mag = cfg.multi_tail_min + u * (available_tail_max - cfg.multi_tail_min)
    tail_mag = np.sort(tail_mag)[::-1]
    tail_sign = rng.choice(np.array([-1.0, 1.0]), size=remaining)
    eigenvalues = np.concatenate([leading, tail_sign * tail_mag])

    A = Q @ np.diag(eigenvalues) @ Q.T
    normality_error = float(np.linalg.norm(A.T @ A - A @ A.T, ord="fro"))
    if normality_error > 1e-10:
        raise RuntimeError("Constructed multi-filter system is not numerically normal.")
    return A, Q, eigenvalues, {
        ...
        "normality_error_fro": normality_error,
        "leading_five_eigenvalues": tuple(float(x) for x in leading),
        "tail_max_abs_eigenvalue": float(np.max(np.abs(eigenvalues[cfg.multi_system_target_modes:]))),
    }
...
def construct_multi_x0(
    ...
    """Exact Experiment-3 balanced excitation: all five designated modes nonzero."""
    rng = np.random.default_rng(state_seed)
    coeff = rng.normal(0.0, cfg.multi_tail_coefficient_scale, size=cfg.dim)
    signs = rng.choice(np.array([-1.0, 1.0]), size=cfg.multi_system_target_modes)
    coeff[: cfg.multi_system_target_modes] = signs  # |a1|=...=|a5|=1
    # Do NOT renormalize: Experiment 3 used x0 = Q @ coefficients directly.
    return Q @ coeff, coeff
...
def summarize_outcomes(rows: pd.DataFrame, cfg: ExperimentConfig) -> pd.DataFrame:
    ...
    for gidx, (group_keys, g) in enumerate(rows.groupby(keys, sort=True, dropna=False)):
        ...
        base = {
            ...
            "n_initial_states_per_system": int(
                g.groupby("system_uid")["initial_state_within_system"].nunique().max()
            ),
...
def summarize_reference_vs_estimated(rows: pd.DataFrame, cfg: ExperimentConfig) -> pd.DataFrame:
    ...
    for gidx, (group_keys, g) in enumerate(use.groupby(keys, sort=True)):
        ...
        comparison_available = n_accepted > 0
        ref_success = _reference_overall_success(g) if comparison_available else np.nan
        est_rel = _reliability(g)
        ref_rel = _reference_reliability(g) if comparison_available else np.nan
        ...
        result.append({
            ...
            "estimated_minus_reference_success_rate": (
                est_success - ref_success if comparison_available and np.isfinite(ref_success) else np.nan
            ),
...
def build_design_table(cfg: ExperimentConfig) -> pd.DataFrame:
    ...
    for case in SINGLE_CASES:
        rows.append({
            ...
            "n_initial_states_per_system": cfg.single_initial_states_per_system,
    ...
    rows.append({
        ...
        "window_lengths": ", ".join(str(x) for x in cfg.window_lengths),
        "n_systems": cfg.system_replicates,
        "n_initial_states_per_system": cfg.multi_initial_states_per_system,
        "n_noise_realizations_nonzero": cfg.noise_realizations_per_nonzero_level,
        "noise_definition": "E||eps_t||^2 = rho^2 ||x0-L||^2",
        "underlying_leading_modes": ", ".join(
            f"{x:g}" for x in cfg.multi_system_leading_eigenvalues
        ),
        "underlying_balanced_excitation": "|a1|=|a2|=|a3|=|a4|=|a5|=1",
    })
    return pd.DataFrame(rows)


def validate_experiment3_noiseless_baseline(table2: pd.DataFrame, cfg: ExperimentConfig) -> None:
    """
    For the prespecified default design, require the corrected rho=0,m=20
    multi-filter branch to reproduce the Experiment-3 moderate-gap/balanced
    selected-output baseline: q1 success=1, q2 success=1, q3 success=0.

    The check is skipped for customized system/state counts, seeds, grids, or
    tolerances because those are no longer the exact Experiment-3 replication.
    """
    exact_design = (
        cfg.seed == 42
        and cfg.system_replicates == 10
        and cfg.multi_initial_states_per_system == 20
        and 20 in cfg.window_lengths
        and 0.0 in cfg.noise_levels
        and abs(cfg.recovery_tolerance_deg - 2.5) < 1e-12
        and tuple(cfg.multi_system_leading_eigenvalues) == (0.96, 0.92, 0.88, 0.84, 0.80)
    )
    if not exact_design:
        print("Experiment-3 baseline sanity check skipped because the default replication design was customized.")
        return

    sub = table2[
        table2["evaluation_type"].eq("multi_filter")
        & table2["case_name"].eq(MULTI_CASE)
        & np.isclose(table2["noise_level_rho"], 0.0)
        & table2["window_length"].eq(20)
    ]
    got = {int(r.stage): float(r.successful_recovery_rate) for r in sub.itertuples()}
    expected = {1: 1.0, 2: 1.0, 3: 0.0}
    if set(got) != set(expected) or any(abs(got[k] - expected[k]) > 1e-12 for k in expected):
        raise RuntimeError(
            "Corrected Experiment-6 multi-filter rho=0,m=20 baseline does not reproduce "
            f"Experiment 3. Expected {expected}, got {got}."
        )
    print("Sanity check passed: rho=0,m=20 reproduces Experiment-3 baseline q1=1, q2=1, q3=0.")
...
def make_figures(out_dir: Path, outcomes: pd.DataFrame, energy: pd.DataFrame, refcomp: pd.DataFrame, cfg: ExperimentConfig) -> None:
    ...
    # Figure 5: remaining squared error.  Raw eta_i is kept in Table 3, but the
    # heatmap uses log10(eta_i) because eta spans many orders of magnitude.
    fig, axes = plt.subplots(1, 3, figsize=(13, 4), constrained_layout=True)
    energy_plot = energy.copy()
    eta = pd.to_numeric(energy_plot.get("median_estimated_eta_local"), errors="coerce")
    energy_plot["log10_median_estimated_eta_local"] = np.where(
        np.isfinite(eta) & (eta > 0.0), np.log10(eta), np.nan
    )
    vals = energy_plot.loc[energy_plot["case_name"].eq(MULTI_CASE), "log10_median_estimated_eta_local"].to_numpy(dtype=float)
    finite = vals[np.isfinite(vals)]
    vmin_eta = float(np.min(finite)) if len(finite) else None
    vmax_eta = float(np.max(finite)) if len(finite) else None
    last_im = None
    for stage, ax in enumerate(axes, start=1):
        sub = energy_plot[(energy_plot["case_name"] == MULTI_CASE) & (energy_plot["stage"] == stage)]
        last_im = _heatmap(
            ax, sub, "log10_median_estimated_eta_local", f"q{stage}: log10 eta_{stage}",
            cfg.noise_levels, cfg.window_lengths, vmin=vmin_eta, vmax=vmax_eta, fmt=".2f",
        )
    fig.suptitle("Experiment 6: log10 remaining squared-error fraction after each filter")
    if last_im is not None:
        fig.colorbar(last_im, ax=axes, shrink=0.8, label="log10 median eta_i")
    fig.savefig(out_dir / "figure5_remaining_squared_error_heatmaps.png", dpi=220)
    plt.close(fig)

    # Figure 6: only q2 and q3 are informative.  q1 has no preceding deflation,
    # so estimated/reference sequences are identical by construction.
    fig, axes = plt.subplots(1, 2, figsize=(9.5, 4), constrained_layout=True)
    useful = refcomp[(refcomp["case_name"] == MULTI_CASE) & refcomp["stage"].isin([2, 3])]
    vals = useful["estimated_minus_reference_success_rate"].to_numpy(dtype=float)
    finite = vals[np.isfinite(vals)]
    lim = max(0.05, float(np.max(np.abs(finite))) if len(finite) else 0.05)
    last_im = None
    for stage, ax in zip((2, 3), axes):
        sub = useful[useful["stage"] == stage]
        last_im = _heatmap(
            ax, sub, "estimated_minus_reference_success_rate", f"q{stage}",
            cfg.noise_levels, cfg.window_lengths, vmin=-lim, vmax=lim,
            fmt="+.2f", annotate_nan=True,
        )
...
def run_experiment(
    cfg: ExperimentConfig,
    out_dir: Path,
    reuse_single_results_dir: Optional[Path] = None,
) -> None:
    validate_config(cfg)
    out_dir = out_dir.resolve()
    if reuse_single_results_dir is not None and out_dir == reuse_single_results_dir.resolve():
        raise ValueError(
            "Use a NEW --output directory when --reuse-single-results-dir is supplied; "
            "do not overwrite the original single-filter results."
        )
    ...
    if reuse_single_results_dir is not None:
        reuse_single_results_dir = reuse_single_results_dir.resolve()
        old_stage_path = reuse_single_results_dir / "trajectory_stage_outcomes.csv"
        old_base_path = reuse_single_results_dir / "base_trajectories.csv"
        if not old_stage_path.exists() or not old_base_path.exists():
            raise FileNotFoundError(
                "--reuse-single-results-dir must contain trajectory_stage_outcomes.csv "
                "and base_trajectories.csv"
            )
        old_stage = pd.read_csv(old_stage_path)
        old_base = pd.read_csv(old_base_path)
        old_stage = old_stage[old_stage["evaluation_type"].eq("single_filter")].copy()
        old_base = old_base[old_base["evaluation_type"].eq("single_filter")].copy()
        all_rows.extend(old_stage.to_dict("records"))
        trajectory_rows.extend(old_base.to_dict("records"))
        branch_names = [MULTI_CASE]
        print(f"Reusing {len(old_stage)} single-filter stage outcomes from {reuse_single_results_dir}")
        print("Only the corrected multi-filter branch will be simulated.")
    else:
        branch_names = list(SINGLE_CASES) + [MULTI_CASE]

    n_single_branches = sum(branch in SINGLE_CASES for branch in branch_names)
    n_multi_branches = sum(branch == MULTI_CASE for branch in branch_names)
    total_base_orbits = cfg.system_replicates * (
        n_single_branches * cfg.single_initial_states_per_system
        + n_multi_branches * cfg.multi_initial_states_per_system
    )
    base_done = 0

    for system_replicate in range(cfg.system_replicates):
        # Preserve the original Experiment-6 seeds for single-filter branches.
        single_system_seed = cfg.seed + 100_000 * system_replicate + 17
        single_systems = {
            case: build_single_system(cfg, case, single_system_seed)
            for case in SINGLE_CASES
        } if any(branch in SINGLE_CASES for branch in branch_names) else {}

        # Exact Experiment-3 system seed for the corrected multi-filter branch.
        multi_system_seed = cfg.seed + 100_000 * system_replicate
        A_multi, Q_multi, eig_multi, meta_multi = build_multi_system(cfg, multi_system_seed)
        ...
        for case_index, branch in enumerate(branch_names):
            if branch in SINGLE_CASES:
                ...
                system_seed = single_system_seed
                n_states = cfg.single_initial_states_per_system
            else:
                A, Q, meta = A_multi, Q_multi, meta_multi
                evaluation_type = "multi_filter"
                true_basis = Q[:, : cfg.multi_n_directions]
                target_defined_single = True
                system_seed = multi_system_seed
                n_states = cfg.multi_initial_states_per_system

            for state_index in range(n_states):
                if evaluation_type == "single_filter":
                    # Preserve old Experiment-6 single-filter pairing.
                    state_seed = cfg.seed + 10_000_000 + system_replicate * 10_000 + state_index
                    x0, coeff = construct_single_x0(cfg, branch, Q, state_seed)
                else:
                    # Exact Experiment-3 state seed: independent of spectrum/profile.
                    state_seed = cfg.seed + 10_000_000 * system_replicate + state_index
                ...
                trajectory_rows.append({
                    ...
                    "a4": float(coeff[3]) if len(coeff) > 3 else np.nan,
                    "a5": float(coeff[4]) if len(coeff) > 4 else np.nan,
                    "multi_underlying_five_mode_spectrum": (
                        json.dumps(list(cfg.multi_system_leading_eigenvalues))
                        if evaluation_type == "multi_filter" else ""
                    ),
                })
    ...
    validate_experiment3_noiseless_baseline(table2, cfg)
    ...
    manifest = {
        ...
        "notes": [
            ...
            "Corrected multi-filter branch uses the exact Experiment-3 moderate-gap five-mode spectrum (0.96,0.92,0.88,0.84,0.80) with balanced excitation of all five modes.",
            "Figure 5 displays log10(eta_i); Table 3 retains raw eta_i.",
            "Reference-vs-estimated comparison is N/A when no estimated-filter window is selected; q1 is omitted from Figure 6 because there is no preceding deflation.",
        ],
    }
...
def main() -> None:
    ...
    run_experiment(
        cfg,
        args.output.resolve(),
        reuse_single_results_dir=args.reuse_single_results_dir,
    )
